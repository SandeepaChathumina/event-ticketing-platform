...
export default function CinemaHome() {
  ...
  const scheduledMovies = useMemo(() => {
    const now = new Date();
    const activeShowtimes = allShowtimes.filter(s => new Date(s.startTime) >= now);
    const movieIdsWithShowtimes = new Set(activeShowtimes.map(s => s.movie?.id));
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const moviesRes = await axios.get("http://localhost:8080/api/v1/catalog/movies");
        const fetchedMovies = moviesRes.data;
        setMovies(fetchedMovies);

        const showtimePromises = fetchedMovies.map((movie: Movie) => 
  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-neutral-50 relative">
      ...
      {/* NEW: Cinematic Video Hero Section */}
      {view === "movies" && (
        <section className="relative w-full h-[75vh] min-h-[500px] flex items-center justify-center overflow-hidden mb-16">
          {/* Video Background (Update the src URL to your preferred video) */}
          <video
            autoPlay
            loop
            muted
          ...
          >
            <source src="https://www.pexels.com/download/video/34139441/" type="video/mp4" />
          </video>
          
          {/* Gradient overlay to seamlessly fade into the dark background below */}
          <div className="absolute inset-0 bg-gradient-to-t from-neutral-950 via-neutral-950/20 to-black/70 z-0"></div>

          {/* Content Container */}
          <div className="relative z-10 text-center px-6 w-full max-w-7xl mx-auto mt-20">
            <motion.h1
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              className="text-5xl md:text-7xl lg:text-8xl font-black mb-6 tracking-tighter text-white drop-shadow-2xl"
            >
              GREAT STORIES. <br className="md:hidden" /><span className="text-rose-600">BIG SCREEN.</span>
            </motion.h1>
            <motion.p
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              transition={{ delay: 0.2 }}
              className="text-lg md:text-xl text-neutral-300 font-medium max-w-2xl mx-auto drop-shadow-lg"
            >
              Immerse yourself in the ultimate cinematic experience. Book your tickets now for the latest blockbusters.
            </motion.p>
          </div>
        </section>
      )}

      {/* Main Content Area - Padding adjusts based on whether the hero is present */}
      <main className={`max-w-7xl mx-auto px-6 pb-12 ${view === "movies" ? "pt-0" : "pt-32"}`}>
        {view === "dashboard" ? (
          <AdminDashboard />
        ) : (
          <>
            ...
            {view === "browse" && <BrowseMovies movies={scheduledMovies} onSelectMovie={handleSelectMovie} />}
